...
router.put('/profile/avatar', authenticate, (req, res) => {
    try {
...
router.post('/reset-password', (req, res) => {
    try {
        ...
        // Mark token as used
        db.run('UPDATE password_reset_tokens SET used = 1 WHERE id = ?', [resetRecord.id]);
...
router.get('/verification', authenticate, (req, res) => {
    try {
        ...
        const user = db.get('SELECT kyc_status FROM users WHERE id = ?', [req.user.id]);
...
router.post('/verification', authenticate, (req, res) => {
    try {
        ...
        const existing = db.get('SELECT user_id, status FROM user_verification WHERE user_id = ?', [req.user.id]);
...
router.get('/transactions', authenticate, (req, res) => {
    try {
        ...
        const transactions = db.all(dataSql, dataParams);
...
router.post('/close-account', authenticate, (req, res) => {
    try {
        db.run(
